...
grep -q 'DEFAULT_WORKSPACE=/usr/local/agent-studio/data/workspaces$' "$base/templates/agent-api.env.template"
grep -q 'ORG_SYNC_ENABLED=false' "$base/templates/agent-api.env.template"
grep -q '@api path /api/\* /healthz' "$base/templates/Caddyfile.template"
