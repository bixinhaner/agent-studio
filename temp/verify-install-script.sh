...
grep -q 'ensure_integration_bootstrap' "$script"
...
grep -q 'Configure DingTalk OAuth now?' "$script"
grep -q "Record OpenAI/Codex install hints now?" "$script"
grep -q "Record a Zendesk site URL now?" "$script"
...
grep -q 'dingtalk auth:' "$script"
grep -q 'openai/codex install hint:' "$script"
grep -q 'zendesk site hint:' "$script"
