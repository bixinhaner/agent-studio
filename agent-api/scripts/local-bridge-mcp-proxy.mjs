...
const token = process.env.AGENT_STUDIO_LOCAL_BRIDGE_TOKEN || "";
...
process.stdin.on("data", (chunk) => {
  ...
  drainFrames().catch((error) => {
    writeError(null, error instanceof Error ? error.message : "Local computer MCP proxy failed");
  });
});
...
async function handleMessage(body) {
  ...
  if (!baseUrl || !token) {
    writeError(message.id ?? null, "Local computer MCP proxy is not configured");
    return;
  }

  const requestId = randomUUID();
  try {
    const response = await fetch(`${baseUrl}/api/local-bridge/mcp/rpc`, {
      method: "POST",
      headers: {
        authorization: `Bearer ${token}`,
        "content-type": "application/json"
      },
      body: JSON.stringify({ rpc: message, request_id: requestId }),
      signal: AbortSignal.timeout(30000)
    });
    const payload = await response.json().catch(() => undefined);
    if (!response.ok) {
      writeError(message.id ?? null, payload?.detail || `Local computer MCP proxy HTTP ${response.status}`);
      return;
    }
    writeFrame(payload);
  } catch (error) {
    writeError(message.id ?? null, `${error instanceof Error ? error.message : "Local request failed"}. Execution outcome may be pending. Check local_request_result with request_id=${requestId} before repeating this operation.`);
  }
}
