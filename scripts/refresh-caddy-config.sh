...
Options:
  ...
  --caddy-upstream-port <port>  Backend port used by Caddy reverse proxy [default: $CADDY_UPSTREAM_PORT]
...
main() {
  ...
  render_caddy_config "$CADDY_TEMPLATE_FILE" "$rendered_config" "$DOMAIN" "$APP_UI_DIR/dist" "$CADDY_UPSTREAM_HOST" "$CADDY_UPSTREAM_PORT"
