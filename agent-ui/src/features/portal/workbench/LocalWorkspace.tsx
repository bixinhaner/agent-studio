...
export function LocalWorkspaceControls() {
  ...
  const trigger = <button type="button" className={`local-workspace-trigger${local.selection ? ' has-selection' : ''}${local.offline ? ' is-offline' : ''}`} aria-label="使用电脑文件夹" aria-expanded={open} title={local.selection ? `${local.selection.device_name}\n${local.selection.path}` : '云端工作区 · 点击使用电脑文件夹'} onClick={() => setOpen(!open)}>{local.selection ? <Folder size={16} /> : <Cloud size={16} />}<span>{local.selection ? `本机 · ${local.selection.label}` : '云端工作区'}</span>{local.offline ? <small>离线</small> : null}<ChevronDown size={13} /></button>;
