  ...
  return <Drawer open={props.open} onClose={props.onClose} width={560} title="我的电脑" rootClassName="local-bridge-drawer">
    ...
    <div className="local-bridge-downloads">
      ...
      <div className="local-bridge-download-grid">
        ...
        ].map(([platform, description, file]) => <a className="local-bridge-download-card" href={`https://github.com/bixinhaner/agent-studio/releases/latest/download/${file}`} target="_blank" rel="noreferrer" key={platform}><span className="local-bridge-download-icon"><Download size={16} /></span><span><strong>{platform}</strong><small>{description}</small></span><span className="local-bridge-download-arrow">↗</span></a>)}
      </div>
