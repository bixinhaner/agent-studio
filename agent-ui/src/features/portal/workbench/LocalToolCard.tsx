...
export function LocalToolCard(props: { toolName?: string; result?: unknown; status?: { type?: string }; args?: Record<string, unknown> }) {
  ...
  const running = props.status?.type === 'running' || output === undefined;
  ...
  return <div className="local-tool-card"><div className="local-tool-status"><Monitor size={17} /><span>{running ? '正在' : '已在'}{local?.selection?.device_name || '用户电脑'}上{label}</span>{running ? <LoaderCircle size={15} className="local-spinner" /> : output?.ok === false ? <AlertCircle size={15} /> : <Check size={15} />}</div>{filePath ? <div className="local-tool-file"><File size={18} /><strong title={filePath}>{filePath.split(/[\\/]/).pop()}</strong><button type="button" onClick={() => void open()} disabled={opening || local?.offline || !local?.selection?.thread_id}>{opening ? '正在打开…' : '在电脑上打开'}<SquareArrowOutUpRight size={13} /></button></div> : null}{error || output?.ok === false ? <p role="status">{error || (output.pending ? '等待电脑返回结果' : output.error)}</p> : null}</div>;
}
