...
Built-in xOMC reports now reuse one Action Connector conversation for the same scenario, managed-resource partition, connector identity, locale and effective authorization. The event's `conversationScope.partition` is the source-provided partition; older events fall back to a stable device/group/resource partition. Event IDs, alarm IDs, task IDs, timestamps and run attempts do not become conversation identity. The connector's live policy digest is added by Studio before execution, so a permission or policy change rotates the context. Stable conversations remain active for reuse; old per-run conversations are archived after terminal delivery, and idle stable conversations can be archived and reopened by the retention operation. To archive legacy terminal threads created before this behavior was deployed, run `npm run threads:archive-legacy-proactive` for a read-only preview and add `-- --apply` only after reviewing the candidate count.
